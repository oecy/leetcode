...
class Solution {
    public Node cloneGraph(Node node) {
        if(node == null)
            return node;//特殊情况优先处理掉
        //因为是无向图 所以必须要有访问标记 用来防止死循环
        //同时hashmap也可以同时将复制好的新节点与对应的节点存在一起
        ...
        while(!q.isEmpty()){
            ...
            for(Node cur : n.neighbors){
                //如果邻接节点没有被访问过,将其加入已访问map，同时将其放入队列中
                if(!visited.containsKey(cur)){
                    visited.put(cur, new Node(cur.val,new ArrayList()));
                    q.add(cur);
                }
                //将复制的邻接节点放入到复制节点的ner中
                visited.get(n).neighbors.add(visited.get(cur));
            }
